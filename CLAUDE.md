...
- **KaTeX `$…$` / `\(…\)` renderuje się tylko w węzłach tekstowych DOM** — nie przetwarza składni LaTeX w CSS `content` pseudo-elementów (`::before`, `::after`), w atrybutach HTML (`title`, `alt`, `aria-label`, `placeholder`, `value`) ani wewnątrz SVG `<text>` / `<tspan>`. Pseudo-element z `content:"$a^n$"` pokaże w tle literalny tekst `$a^n$`. Do dekoracji i etykiet w tych kontekstach używaj Unicode (superskrypty `aⁿ b² x⁻¹`, subskrypty `H₂O CO₂`, znaki matematyczne `± × ÷ √ ∑ π`) albo — w SVG — buduj zapis przez `<tspan>` z atrybutami `font-size`, `dy`, `baseline-shift`, nigdy nie wstawiając surowego `$…$` do węzła tekstowego SVG
